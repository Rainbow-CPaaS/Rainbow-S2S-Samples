...
class RoomApi
{
     ...
     * @return \Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus
     */
    public function roomJoin($cnx_id, $room_id)
    {
        list($response) = $this->roomJoinWithHttpInfo($cnx_id, $room_id);
        return $response;
    }

    /**
     * Operation roomJoinWithHttpInfo
     *
     * Join a room
     *
     * @param  string $cnx_id Connection id (required)
     ...
     * @throws \Ale\Rainbow\S2S\Client\S2S\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of \Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus, HTTP status code, HTTP response headers (array of strings)
     */
    public function roomJoinWithHttpInfo($cnx_id, $room_id)
    {
        $request = $this->roomJoinRequest($cnx_id, $room_id);
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus' === '\SplFileObject') {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, '\Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = '\Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus';
            $responseBody = $response->getBody();
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        '\Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus',
     ...
     */
    public function roomJoinAsync($cnx_id, $room_id)
    {
        return $this->roomJoinAsyncWithHttpInfo($cnx_id, $room_id)
            ->then(
                function ($response) {
                    return $response[0];
                }
    ...
    /**
     * Operation roomJoinAsyncWithHttpInfo
     *
     * Join a room
     *
     * @param  string $cnx_id Connection id (required)
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function roomJoinAsyncWithHttpInfo($cnx_id, $room_id)
    {
        $returnType = '\Ale\Rainbow\S2S\Client\S2S\Model\RoomStatus';
        $request = $this->roomJoinRequest($cnx_id, $room_id);
    ...
    /**
     * Create request for operation 'roomJoin'
     *
     * @param  string $cnx_id Connection id (required)
     * @param  string $room_id Room id (required)
     *
     ...
     */
    protected function roomJoinRequest($cnx_id, $room_id)
    {
        // verify the required parameter 'cnx_id' is set
        if ($cnx_id === null || (is_array($cnx_id) && count($cnx_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $cnx_id when calling roomJoin'
            );
        }
        // verify the required parameter 'room_id' is set
        if ($room_id === null || (is_array($room_id) && count($room_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $room_id when calling roomJoin'
            );
        }

        $resourcePath = '/api/rainbow/ucs/v1.0/connections/{cnxId}/rooms/{roomId}/join';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
